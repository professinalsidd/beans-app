...
export const BikeScreen = () => {
  ...
  const handleSubmit = () => {
    setLoading(true);
    Axios.get(
      `https://api.api-ninjas.com/v1/motorcycles?make=${inputForm}&model=${inputForm2}`
    )
      .then((res) => {
    ...
    <Fragment>
      ...
      <View style={SINGLE_INPUT_STYLE.container}>
        <View style={SINGLE_INPUT_STYLE.subContainer}>
          <TextInputComponent
            placeHolderName="Enter your bike name"
          ...
          <TextInputComponent
            placeHolderName="Enter your bike modal name"
            placeHolderTextColor="#000"
            onChangeText={(text: React.SetStateAction<string>) =>
              setInputForm2(text)
            }
          ...
          <ButtonComponent
            ...
            buttonContainer={[
              ...
              inputForm && inputForm2 ? {} : { opacity: 0.5 },
            ]}
            disabled={!inputForm && !inputForm2}
            text="Search"
