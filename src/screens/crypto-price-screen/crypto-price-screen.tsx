...
export const CryptoPriceScreen = () => {
  ...
  const handleSubmit = () => {
    setLoading(true);
    Axios.get(`https://api.api-ninjas.com/v1/cryptoprice?symbol=${inputForm}`)
      .then((res) => {
    ...
    <Fragment>
      ...
      <View style={SINGLE_INPUT_STYLE.container}>
        <View style={SINGLE_INPUT_STYLE.subContainer}>
          <TextInputComponent
            placeHolderName="Enter Crypto Price Name"
            placeHolderTextColor="#000"
            onChangeText={(text: string) => setInputForm(text.toUpperCase())}
            value={inputForm}
              ...
              <View style={SINGLE_INPUT_STYLE.mainContainer}>
                <View style={SINGLE_INPUT_STYLE.singleCardContainer}>
                  <Text style={SINGLE_INPUT_STYLE.singleCardContainerText}>
                    Symbol:- {showResult && showResult?.symbol}
                  </Text>
                </View>
                <View style={SINGLE_INPUT_STYLE.singleCardContainer}>
                  <Text style={SINGLE_INPUT_STYLE.singleCardContainerText}>
                    Price:- {showResult && showResult?.price}
                  </Text>
                </View>
                <View style={SINGLE_INPUT_STYLE.singleCardContainer}>
                  <Text style={SINGLE_INPUT_STYLE.singleCardContainerText}>
                    Times Tamp:- {showResult && showResult?.timestamp}
                  </Text>
                </View>
              </View>
            </ScrollView>
