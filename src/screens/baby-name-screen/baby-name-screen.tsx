...
export const BabyScreen = () => {
  ...
  const handleSubmit = () => {
    setLoading(true);
    Axios.get(`https://api.api-ninjas.com/v1/babynames?gender=${inputForm}`)
      .then((res) => {
  ...
  const renderItem = ({ item }: { item: string }) => {
    return (
      <View style={SINGLE_INPUT_STYLE.mainContainer}>
        <Text style={SINGLE_INPUT_STYLE.textStyle}>{item}</Text>
      </View>
    );
  };
    ...
    <Fragment>
      <View style={SINGLE_INPUT_STYLE.container}>
        <View style={SINGLE_INPUT_STYLE.subContainer}>
          <TextInputComponent
            placeHolderName="Enter your gender"
          ...
          {errorMsg ? (
            <ErrorComponent
              msg={
                errorMsg
                  ? "Your should type boy or girl not male or female!"
                  : null
              }
            />
          ) : null}
